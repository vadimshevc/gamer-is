...
const ProductCard = memo(({ product, onAdd }) => {
  ...
  return (
    <div className={`bg-slate-900 border ${isOutOfStock ? 'opacity-60 grayscale' : 'border-slate-800'} p-6 rounded-3xl hover:border-indigo-500 transition-all group flex flex-col h-full`}>
      ...
      <div className="w-full aspect-square mb-4 overflow-hidden rounded-2xl bg-slate-800 flex items-center justify-center">
        {product.image ? (
          <img 
            src={product.image} 
            alt={product.name} 
      ...
      <div className="mt-auto pt-4 border-t border-slate-800/50">
        ...
        <button
          ...
          className={`w-full py-4 rounded-2xl font-black transition-all text-white shadow-lg active:scale-95 ${
            isOutOfStock ? 'bg-slate-800 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500'
          }`}
        >
...
function App() {
  ...
  const loadData = async () => {
    setLoading(true);
    setErrorInfo(null);
    logger.info('UI', 'Завантаження товарів з Cloud Firestore');

    try {
      const cloudData = await fetchProductsFromCloud();
      if (cloudData.length === 0) throw new Error('Firestore Error');
  ...
  const handleCheckout = async (phone) => {
    setLoading(true);
    try {
      const GAS_URL = "https://script.google.com/macros/s/AKfycbxOyH3tOYclejqgQ9r5OnRVWKNyALeH9eu3FB-T0ZAbq2rjh963QjF0ZiqQdKROOg5k/exec";
  ...
  const [maxPrice, setMaxPrice] = useState(15000); // Максимальна початкова ціна
      ...
      <header className="max-w-6xl mx-auto mb-12 flex justify-between items-center border-b border-slate-800 pb-6">
        ...
        <div className="text-slate-400 font-medium italic">
          {loading ? 'З’єднання з базою...' : `В ефірі: ${products.length} товарів`}
              ...
              {/* Hero Section */}
<section className="max-w-6xl mx-auto mb-20 pt-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
  <div className="space-y-8 text-center lg:text-left">
    ...
    <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
      <button className="bg-indigo-600 px-10 py-5 rounded-2xl font-black text-white hover:bg-indigo-500 transition-all shadow-[0_0_30px_rgba(79,70,229,0.3)] active:scale-95">
        ПЕРЕЙТИ ДО КАТАЛОГУ
      </button>
  ...
  <div className="relative group">
    ...
    <section className="max-w-6xl mx-auto mb-10 p-1 bg-gradient-to-r from-indigo-500/20 via-slate-800/50 to-purple-500/20 rounded-[2rem] shadow-[0_0_50px_-12px_rgba(79,70,229,0.3)]">
  <div className="bg-slate-950/80 backdrop-blur-xl p-8 rounded-[1.8rem] border border-slate-800/50 flex flex-wrap items-end gap-8">
    ...
    <button 
      onClick={() => { setCategoryFilter('Всі'); setBrandFilter('Всі'); setMaxPrice(20000); }}
